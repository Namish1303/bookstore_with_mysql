...
def addBook(request,*args,**kwargs):
    return render(request,"add_a_book.html",{})


def addB(request,*args,**kwargs):
    req = request.POST
    title = req["Title"]
    pages = req["Pages"]
    if(pages == ''):
    ...
    ISBN = req["ISBN"]
    genre = req["genre"]
    values =(title,int(pages),float(price),author,ISBN,genre)
    sql = 'Insert into book(Title,Pages_number,Price,Author,ISBN,Genre) values(%s,%s,%s,%s,%s,%s)'
    mycursor.execute(sql,values)
    mydb.commit()
    return render(request,"report.html",{})